...
`
    ...
    // Add more if needed, keeping it short for brevity in this turn
];
...
`;
        ...
        for (const template of TEMPLATES) {
            ...
            let safeContent = template.content.replace(/'/g, "''");
            // Remove newlines to avoid SQL issues, or keep them? multiline strings in SQL are fine if enclosed in '
            // But formatting is cleaner if newlines are real.
            // safeContent = safeContent.replace(/\n/g, "\\n"); 

            sqlOutput += `INSERT INTO document_templates (title, category, content) VALUES ('${template.title}', '${template.category}', '${safeContent}');\n`;
        }

        fs.writeFileSync('seed_data.sql', sqlOutput);
        console.log('seed_data.sql generated.');
    } catch (err) {
